...
<script lang="ts" setup>
  ...
  const handleClick = async(remark, val) => {
    ...
    if(remark == 'out'){
      let list = state.list.map(v => {return {timestamp: proxy.parseTime(v.timestamp), value: v.value}})
      let codename = proxy.find(state.meas, ['code', state.code], 'name')
      let name = `${publicStore.active.name} - ${codename}`
