<template>
  <div class="layout-col">
    <!-- 标题 -->
    <aa-title :title="isNull(publicStore.active)?'':publicStore.active.name + ' - 权限配置'">
    ...
    <div class="layout-col white-rgba50 rad8 p15">
      <div class="flex-sc mb10">
        <div class="rad4 ptb5 plr8 flex-cc cursor bgi1 white ml10 bo-i1-1" @click.stop="state.auths.map(a=>a.select = true)">
          <i-ep-connection class="f12 fw" /><span class="f14 ml2">全选</span>
        </div>
        <div class="rad4 ptb5 plr8 flex-cc cursor bgi1 white ml10 bo-i1-1" @click.stop="state.auths.map(a=>a.select = false)">
