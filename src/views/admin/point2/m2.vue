  ...
  <div class="layout-col">
    ...
    <aa-title :title="isNull(publicStore.active)?'':publicStore.active.name">
      ...
      <template #right-content>
        ...
        <div class="rad4 ptb5 plr8 flex-cc cursor bgi1 white ml10 relative" @click.stop="state.tempele = !state.tempele">
          <i-ep-compass class="f12 fw" /><span class="f14 ml5">模板</span>
          <div class="rad4 bgd bo-ce-1 ca absolute l0 t40 w100 z1 flex-col-cs" v-if="state.tempele">
            <div class="hover1 plr8 ptb8 cursor ww100"  @click.stop="handleClick('tempeleOn')">设置模板</div>
            <div class="bot-ce-1 ww100 "></div>
        ...
        <div class="rad4 ptb5 plr8 flex-cc cursor bgi1 white ml10" @click.stop="handleClick('dels')">
    ...
    <div class="layout-col white-rgba50 rad8 p15">
      ...
      <Pagination style="padding-bottom: 0px;" v-show="state.total>0" :total="state.total" v-model:page.sync="state.page" v-model:limit.sync="state.limit" @pagination="init" />
    </div>
    <!-- 底部 -->
    <aa-foot></aa-foot>
...
<script lang="ts" setup>
  ...
  let datatypes = [
    ...
    {name: 'U64-H', value: 'U64-H'},
    {name: 'U64-L', value: 'U64-L'},
    {name: 'S64-H', value: 'S64-H'},
    {name: 'S64-L', value: 'S64-L'},
  ]
  let lens = [
    {name: '0', value: '0'},
    {name: '1', value: '1'},
  ...
  const state = reactive({
    ...
    addItem: {},
    model: 't_v_modbus_point',
    modelType: 't_v_type_template',
    ruleList: {},
  })

	onMounted(async() => {
    createScheduled('modbus', 1*1000, () => { getReal() })
	})
  ...
  const getReal = async() => {
    ...
    res.forEach(v => {
      ...
      if(!proxy.isNull(state.scale) && state.scale.code){
  ...
  // 模拟数据
  const mock = async(res) => {
    let data = state.list.find(a=>a.type == 'METER' && a.code == 'hour_electrlc')
    if(data){
      // console.log("data",  data)
      if ('All_active_electric' in res[0]) {
        let timestamp = res[0]['timestamp']
        let value = +res[0]['All_active_electric']
        // 判断是否存在
        let query1 = {model: 't_sensor_measure_float', args: `sensor_id='${data.sensorid}' and measurement_code='${data.code}' and CAST(timestamp AS UNSIGNED)<=${timestamp}`, limit: 1}
        let res1 = await publicStore.http({Api: query1})
        if(!proxy.isNull(res1) && (Math.abs(res1[0]['timestamp'] - Number(timestamp)) < 1000)){
          // console.log("数据库存在---res1", res1 )
          data.value = Math.floor(100*Number(res1[0]['value']))/100
        }else{
          // 不存在数据库
          let query2 = {model: 't_sensor_measure_float', args: `sensor_id='${data.sensorid}' and measurement_code='All_active_electric' and CAST(timestamp AS UNSIGNED)>${Number(timestamp)-3600*1000}`, limit: 1, order: '(timestamp + 0) ASC'}
          let res2 = await publicStore.http({Api: query2})
          // console.log("数据库不存在---res2", res2)
          if(!proxy.isNull(res2)){
            data.value =  Math.floor(100*Math.abs(value - Number(res2[0]['value'])))/100
            // console.log(`模拟的值是: ${res2}`)
            let querys = [{sensor_id: data.sensorid, measurement_code: data.code, timestamp: timestamp, value: data.value}]
            let params = {query: querys, ip: proxy.getHttp('ip'), port: '10132',  url: "/api/v1.0/p3000/monitorOnline/commFrontEndProcessor/UpdateSensorData/auto"}
            let resx = await publicStore.http({httpApi: params})
  ...
  const handleClick = async(remark, val) => {
    ...
    if(remark == 'crl') {
      ...
      if(val.mode != 2) return ElNotification({ title: '提示', message: '请选择正确模式', type: 'error' })
  ...
  const handleControl = async(item) => {
    if(state.tempKey && state.tempValue == item[state.tempKey]) return
    if(item.mode == 1) return ElNotification({ title: '提示', message: '请选择正确模式', type: 'error' })
