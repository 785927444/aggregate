<template>
  <div class="layout-col flex3 ml16">
    <!-- 标题 -->
		<aa-title :title="isNull(publicStore.active)?'':publicStore.active.name + ' - 控制'">
...
<script lang="ts" setup>
  ...
  const state = reactive({
    ...
    editFrom: [
      ...
      { required: true,  editshow: true, name: '关联', key: 'code', type: 'select', list: [], value: 'code', label: 'name', relation: ['name'] }, 
  ...
  const init = async(key) => {
    if(publicStore.actIndex != 5) return
    ...
    publicStore._public.controlList = state.list
