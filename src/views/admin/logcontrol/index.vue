<template>
  <div class="layout-col">
    <!-- 标题 -->
    <aa-title title="系统日志" class="mb10">
      ...
      <template #right-content>
        <div class="flex-ec cursor flex1">
          ...
          <el-button class="rad4 ptb5 plr8 flex-cc cursor bgi1 white ml10" @click.stop="state.page = 1; init()" :icon="Search">搜索</el-button>
          <el-button class="rad4 ptb5 plr8 flex-cc cursor bgi1 white ml10" @click.stop="exports()" :icon="Filter">导出</el-button>
        </div>
      </template>
    </aa-title>
    <!-- 内容 -->
...
<script lang="ts" setup>
  ...
  const state = reactive({
    ...
    content: [
      ...
      { width: 'w50x3', show: true, align: 'left', key: 'instlPos', name: '安装位置' },
      { width: 'w50x3', show: true, align: 'left', key: 'code_name', name: '控制名称' },
      { width: 'w50x3', show: true, align: 'left', key: 'action_name', name: '动作名称' },
      { width: 'w50x3', show: true, align: 'left', key: 'userName', name: '操作人员', type: 'decrypt' },
      { width: 'w50x6', show: true, align: 'left', key: 'orther', name: '操作内容' },
  ...
  const getInit = async () => {
    let query = !configStore.distributId || configStore.distributId == '0'? {model: "t_sensor", args: `class='0'`} : {model: "t_sensor", args: `stationNum='${configStore.distributId}' and class='0'`}
  ...
  const init = async(key?: any) => {
    let args = ''
    if(state.sensor_id) args += `sensor_id='${state.sensor_id}' and `
