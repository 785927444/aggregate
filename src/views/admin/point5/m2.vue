...
<script lang="ts" setup>
  ...
  const state = reactive({
    ...
    addItem: {},
    model: 't_v_lonwork_point',
    modelType: 't_v_type_template',
    ruleList: {},
  })

	onMounted(async() => {
    createScheduled('lonwork', 1*1000, () => { getReal() })
    let content1 =  [
      ...
      { width: 'w50x5', show: true, align: 'left', key: 'value', name: '实时值', type: 'select_input'  },
    ...
    state.content = !proxy.isNull(configStore.user) && configStore.user.role_id == '1'? content2 : content1
	})
  ...
  const handleClick = async(remark, val) => {
    ...
    // 配置电表
    if(remark == 'set'){
      if(proxy.isNull(publicStore.active)) return ElNotification({ title: '提示', message: '请选择设备', type: 'error' })
      if(proxy.isNull(publicStore.active.comm_params)) return ElNotification({ title: '提示', message: '请配置参数', type: 'error' })
      let active = {...publicStore.active}
      try {
        active.comm_params = JSON.parse(active.comm_params)
      } catch (error) {
        console.error("解析失败:", error.message)
      }
      active.sensorid = publicStore.active.id + ''
      active.code = 'DBID'
      active.value = active.comm_params[active.code]
      controlRef.onVisable(active)
    }
  }
