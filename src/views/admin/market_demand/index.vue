...
<script lang="ts" setup>
  ...
  const state = reactive({
    ...
    content: [
      ...
      { width: 'w50x2', show: true, align: 'left',  key: 'title_name', name: '交易名称'},
      { width: 'w50x2', show: true, align: 'left',  key: 'transaction_id', name: '交易编号'},
      { width: 'w50x2', show: true, align: 'left',  key: 'source', name: '发布来源' },
      { width: 'w50x2', show: true, align: 'left',  key: 'demand_type', name: '需求类型' },
      { width: 'w50x2', show: true, align: 'left',  key: 'demand_area', name: '需求地区' },
      { width: 'w50x2', show: true, align: 'left',  key: 'response_type', name: '响应类型' },
      { width: 'w50x2', show: true, align: 'left',  key: 'response_date', name: '响应日期', type: 'time' },
      { width: 'w50x2', show: true, align: 'left',  key: 'demand_time_slot', name: '需求时段', type: 'time' },
      { width: 'w50x2', show: true, align: 'left',  key: 'market_demand_capacity', name: '市场需求容量' },
      { width: 'w50x2', show: true, align: 'left',  key: 'apply_deadline', name: '报名截止时间',type: 'time' },
      { width: 'w50x3', show: true, align: 'left',  key: 'event_status', name: '事件状态',type: 'select', list: statusEvent, value: 'value', label: 'name' },
    ...
    type: '',
    level: '',
    model: 't_trade_demand',
  })

  onMounted(async() => {
    publicStore.init({path: '/trade_demand'}, state)
  ...
  const init = async() => {
    state.query = !configStore.distributId || configStore.distributId=='0'? {model: state.model, order: 'apply_deadline desc'} : {model: state.model, order: 'apply_deadline desc'}
    let args = ''
    ...
    console.log('state.parmasstate.parmasstate.parmas',state.parmas)
