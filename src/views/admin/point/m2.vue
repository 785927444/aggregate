  ...
  const handleClick = async(remark, val) => {
    ...
    if(remark == 'tempeleOn'){
      ElMessageBox.confirm(`是否确定把【${publicStore.active.name}】设置成模板?`, '温馨提示', {confirmButtonText: '确定', cancelButtonText: '关闭', type: 'success'}).then(async() => {
        let query = {model: state.modelType, args: `sensorid='${publicStore.active.id}' and class='1'`}
        ...
        let from = {id: proxy.isNull(res)? '' : res[0].id, class: '1', qztype: publicStore.active.qztype, type: publicStore.active.type, name: publicStore.active.name, sensorid: publicStore.active.id + '', value: JSON.stringify(value)}
