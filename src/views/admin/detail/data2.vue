<template>
  <div class="layout-col">
    <!-- 标题 -->
		<aa-title :title="(!isNull(publicStore._public.meas)&&publicStore.text?find(publicStore._public.meas, ['code', publicStore.text], 'name'):'数据趋势')">
...
<script lang="ts" setup>
  ...
  async function setChart(data){
    ...
    if(publicStore.text && !proxy.isNull(publicStore._public.cfgs)){
      let cfg = publicStore._public.cfgs.find(a=>a.code == publicStore.text)
      if(cfg && cfg.calcRuleExpres){
        try{
          let calcRuleExpres = JSON.parse(cfg.calcRuleExpres)
          if(calcRuleExpres.dw) attr.y_name = calcRuleExpres.dw
        }catch(e){
          console.log(e)
        }
      }
