...
router.beforeEach(async (to: any, from: any, next) => {
  ...
  if (configStore().isRoute && configStore().isRoute != 'false' && configStore().routes.length == 0 && !to.meta.next) {
    let route_auth:any = {}
    // 厂站 | 获取路由数据+权限数据
    if(isNull(configStore().stations)){
      let res = await publicStore().http({Api1: {model: "role_route_auth"}, Api2: {model: 't_station', args: `parent_id='0'`}})
      route_auth = res.Api1
      configStore().stations = res.Api2.sort((a, b) => a.order - b.order)
    }else{
      route_auth = await publicStore().http({Api: {model: "role_route_auth"}})
    }
    // 生成动态路由
    if(route_auth.length != 0){
      let routes = filterRoutes(route_auth.routes.sort((a:any, b:any) => a.order - b.order), 'id', 'parent_id', 'children', '')
      // 动态挂载路由
      routes.forEach((route:any) => {router.addRoute(route)})
      configStore().routes = routes
      // 获取权限列表
      configStore().auths = route_auth.auths
      // 需登录 无需验证token
      if (to.meta.next) {
        return next()
      } else {
