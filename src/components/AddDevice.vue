<template>
  <el-dialog v-model="visible" :title="title" :before-close="onVisable" :draggable="true" :width="props.state.editFrom&&props.state.editFrom.length>8?'60%':'40%'">
    <el-form ref="formRef" :model="dataForm" :rules="ruleList" label-width="20%">
      <div class="flex-sc warp ww100">
        ...
        <div v-for="(item, index) in editFromInfo.filter(a=>a.editshow!='none')" :key="index" :style="{width: props.state.editFrom&&props.state.editFrom.length>8? (100/2) + '%' : 100 + '%' }" >
          <div v-if="!item.condit || dataForm[item.condit[0]]==item.condit[1]">
            <el-form-item :label="item.name" :prop="item.key" v-if="item.editshow != 'none' && (title == '创建' || item.editshow)">
              ...
              <div v-if="item.type=='json'">
                <div class="rad4 ptb2 plr15 flex-cc cursor bgi1 white" @click.stop="jsonParams(item)" v-if="!isNull(dataForm[item.key])">编 辑</div>
                <div class="rad4 ptb2 plr15 flex-cc cursor bga white" @click.stop="jsonParams(item)" v-else>配 置</div>
              </div>
              <!-- 详情图片 -->
              <div v-if="item.type=='imgs'">
                <div class="rad4 ptb2 plr15 flex-cc cursor bgi1 white" @click.stop="jsonParams(item)" v-if="!isNull(item.value)">编 辑</div>
                <div class="rad4 ptb2 plr15 flex-cc cursor bga white" @click.stop="jsonParams(item)" v-else>配 置</div>
              </div>
              <span class="rad4 ptb2 plr15 flex-cc cursor bgi1 white ml15" @click="handleSync('comm_params')" v-if="dataForm[state.key] && item.key == 'comm_params'">同 步</span>
              <!-- 额外 -->
...
<script lang="ts" setup>
    ...
    editFrom.forEach(item => {
      ...
      if(item.type == 'time') {
        ...
        if(item.editshow || title == '创建'){
          value = proxy.generateRandomString(5) + sCryptoJS.MD5(data.toString().trim()).toString().toUpperCase() + proxy.generateRandomString(5)
        }
  ...
  const handleSubmit = (formEl) => {
    formEl.validate (async valid => {
      if (valid) {
        ...
        let form = getForm()
        // 特殊处理
        if(form.parent_id && !proxy.isNull(publicStore._public.nodes)){
         let node = publicStore._public.nodes.find(a=>a.id == form.parent_id)
         if(node){
          form.stationNum = node.stationNum
          form.bayNum = form.bayNum?form.bayNum:node.bayNum
          form.comm_params = node.comm_params
         }
         console.log("publicStore._public.nodes---", publicStore._public.nodes)
  ...
  const setLog = async(res, mark) => {
    let msg = ''
    if(!proxy.isNull(route.matched)) {
  ...
  const setForm = async(from) => {
    ...
    if(props.state.model == 't_user' && title == '修改') {
      let query = {model: "redis", key: `${from.id}`, value: '0', time: 1}
      let redis = await publicStore.http({setApi: query})
      console.log("退出成功redis", redis)
      if(!proxy.isNull(configStore.user) && configStore.user.id && configStore.user.id == from.id){
